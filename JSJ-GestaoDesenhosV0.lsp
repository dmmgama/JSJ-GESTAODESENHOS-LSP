...
(defun ProcessManualReview ( / loop drawList i item userIdx selectedHandle field revLet revData revDesc dateWarning desNum doc)
  ...
  (if (null drawList)
    ...
    (progn
      (WriteLog "MODIFICAR MANUAL: Iniciado")
      (setq loop T)
      (while loop
        (setq drawList (GetDrawingList))
        (textscr)
        ...
        (cond
          ...
          ((and (> userIdx 0) (<= userIdx i))
            ...
            (cond
              ...
              ((= field "3")
                (initget "A B C D E")
                (setq revLet (getkword "\nQual a Revisao? [A/B/C/D/E]: "))
                (if revLet
                  (progn
                    (setq revData (getstring T "\nData (DD-MM-YYYY): "))
                    (setq dateWarning (ValidateNewRevisionDate selectedHandle revLet revData))
                    (if dateWarning
                      (progn
                        (princ (strcat "\n*** " dateWarning " ***"))
                        (initget "Sim Nao")
                        (if (= (getkword "\nContinuar mesmo assim? [Sim/Nao] <Nao>: ") "Nao")
                          (setq revLet nil)
                        )
                      )
                    )
                    (if revLet
                      (progn
                        (setq revDesc (getstring T "\nDescricao: "))
                        (UpdateSingleTag selectedHandle (strcat "DATA_" revLet) revData)
                        (UpdateSingleTag selectedHandle (strcat "DESC_" revLet) revDesc)
                        (UpdateSingleTag selectedHandle (strcat "REV_" revLet) revLet)
                        (UpdateAttributeR selectedHandle)
                        (vla-Regen doc acActiveViewport)
                        (WriteLog (strcat "MODIFICAR: Des " desNum " - REV_" revLet " (" revData ") " revDesc))
                      )
                    )
                  )
                )
